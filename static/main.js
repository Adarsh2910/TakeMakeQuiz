...
$('[data-toggle="tooltip"]').tooltip(); 
	...
	$('#regbut').click(function(e){
		...
		var formData = {
	 	...
	 	$.ajax({	
		 	type: 'POST',
		 	url: 'http://localhost:8095/user/register',
		 	data: formData,
		 	datatype: 'json',
		 	encode: 'true',
		 }).done(function(res){
	...
	$('#take').click(function(e){
		...
		$.ajax({
			type: 'GET',
		 	url: 'http://localhost:8095/user/quiz/taken/fetch',
	...
	function showQuizes(offset) {
		$.ajax({
			type: 'GET',
		 	url: 'http://localhost:8095/user/quiz/fetch/?offset='+offset,
		}).done(function(res) {
			...
			$.map(takecard, function(value , key ){
				...
				var cardName = document.createElement("DIV");
		 		...
		 		card[0].appendChild(cardName);
					...
					$.ajax({
						type: 'POST',
					 	url: 'http://localhost:8095/user/quiz/take',
					 	data: {quizID : quizid},
					 	datatype: 'json',
						encode: 'true',
					}).done(function(res){
			    ...
			    var s = checkSecond((timeArray[1] - 1));
				   ...
				   $('#takequiz').css("display", "none");
					...
					if(!store.isFinish()) {
						$.ajax({
							type: 'POST',
						 	url: 'http://localhost:8095/user/quiz/finish',
						 	data: {
	...
	$("#finishQuiz").click(function(e){
		...
		$.ajax({
			type: 'POST',
			url: 'http://localhost:8095/user/quiz/add',
		 	data: formData,
		 	datatype: 'json',
		 	encode: 'true',
		}).done(function(res){
	...
	$('#madequiznote').css("display", "none");
		...
		$.ajax({
			type: 'GET',
		 	url: 'http://localhost:8095/user/quiz/created/fetch',
		}).done(function(response){
